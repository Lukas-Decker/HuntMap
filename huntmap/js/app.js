...
(function () {
  ...
  function boot() {
    S.lang  = store(LS.lang)  || (navigator.language || 'en').slice(0, 2).toLowerCase();
    S.photo = store(LS.photo) || 'all';
    S.marks = store(LS.marks) || {};
    Object.assign(S.opts,  store(LS.opts)  || {});
    Object.assign(S.extra, store(LS.extra) || {});
    ...
    Promise.all([
      ...
      var hash = parseHash();
      var start = hash.m || store(LS.map) || 1;
  ...
  function wireUI() {
    ...
    $$('#langMenu button').forEach(function (b) {
      b.addEventListener('click', function () {
        ...
        S.t = (S.allT[S.lang] && S.allT[S.lang].maps) || {};
        store(LS.lang, S.lang);
